...
int
of_list_meter_config_first(of_list_meter_config_t *list, of_object_t *obj)
{
    int rv;

    of_meter_config_init(obj, list->version, -1, 1);
...
int
of_list_meter_config_next(of_list_meter_config_t *list, of_object_t *obj)
{
    int rv;
...
/**
 * Set up to append an object of type of_meter_config to an of_list_meter_config.
...
int
of_list_meter_config_append_bind(of_list_meter_config_t *list, of_object_t *obj)
{
    return of_list_append_bind(list, obj);
}

/**
 * Append an object to a of_list_meter_config list.
 *
...
int
of_list_meter_config_append(of_list_meter_config_t *list, of_object_t *obj)
{
    return of_list_append(list, obj);
}

/**
 * \defgroup of_list_meter_config of_list_meter_config
 */

/**
 * Create a new of_list_meter_config object
 *
...
of_list_meter_config_new(of_version_t version)
{
    ...
    bytes = of_object_fixed_len[version][OF_LIST_METER_CONFIG];

    if ((obj = of_object_new(OF_WIRE_BUFFER_MAX_LENGTH)) == NULL) {
        return NULL;
    }

    of_list_meter_config_init(obj, version, bytes, 0);

    return obj;
}

/**
 * Initialize an object of type of_list_meter_config.
 *
...
void
of_list_meter_config_init(of_object_t *obj,
    of_version_t version, int bytes, int clean_wire)
{
    LOCI_ASSERT(of_object_fixed_len[version][OF_LIST_METER_CONFIG] >= 0);
    if (clean_wire) {
    ...
    obj->version = version;
    obj->length = bytes;
    obj->object_id = OF_LIST_METER_CONFIG;
