...
int
of_list_bsn_vlan_mac_first(of_list_bsn_vlan_mac_t *list, of_object_t *obj)
{
    int rv;

    of_bsn_vlan_mac_init(obj, list->version, -1, 1);
...
int
of_list_bsn_vlan_mac_next(of_list_bsn_vlan_mac_t *list, of_object_t *obj)
{
    int rv;
...
/**
 * Set up to append an object of type of_bsn_vlan_mac to an of_list_bsn_vlan_mac.
...
int
of_list_bsn_vlan_mac_append_bind(of_list_bsn_vlan_mac_t *list, of_object_t *obj)
{
    return of_list_append_bind(list, obj);
}

/**
 * Append an object to a of_list_bsn_vlan_mac list.
 *
...
int
of_list_bsn_vlan_mac_append(of_list_bsn_vlan_mac_t *list, of_object_t *obj)
{
    return of_list_append(list, obj);
}

/**
 * \defgroup of_list_bsn_vlan_mac of_list_bsn_vlan_mac
 */

/**
 * Create a new of_list_bsn_vlan_mac object
 *
...
of_list_bsn_vlan_mac_new(of_version_t version)
{
    ...
    bytes = of_object_fixed_len[version][OF_LIST_BSN_VLAN_MAC];

    if ((obj = of_object_new(OF_WIRE_BUFFER_MAX_LENGTH)) == NULL) {
        return NULL;
    }

    of_list_bsn_vlan_mac_init(obj, version, bytes, 0);

    return obj;
}

/**
 * Initialize an object of type of_list_bsn_vlan_mac.
 *
...
void
of_list_bsn_vlan_mac_init(of_object_t *obj,
    of_version_t version, int bytes, int clean_wire)
{
    LOCI_ASSERT(of_object_fixed_len[version][OF_LIST_BSN_VLAN_MAC] >= 0);
    if (clean_wire) {
    ...
    obj->version = version;
    obj->length = bytes;
    obj->object_id = OF_LIST_BSN_VLAN_MAC;
