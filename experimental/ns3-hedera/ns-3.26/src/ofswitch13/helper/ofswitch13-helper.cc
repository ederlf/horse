...
OFSwitch13Helper::InstallSwitch (
    Ptr<Node> swNode, uint64_t dpid, std::vector< std::pair< Ptr< NetDevice >, uint32_t> > ports)
{
  NS_LOG_FUNCTION (this << swNode);
  ...
  // Create and aggregate the OpenFlow device to the switch node.
  Ptr<OFSwitch13Device> openFlowDev = m_devFactory.Create<OFSwitch13Device> ();
  /* Unregister and register with the new dpid */
  uint64_t oldid = openFlowDev->GetDatapathId();
  OFSwitch13Device::UnregisterDatapath (oldid);
  openFlowDev->SetDatapathId(dpid);
  OFSwitch13Device::RegisterDatapath (dpid, openFlowDev);
  ...
  // Add switch ports.
  for(auto port: ports){
    openFlowDev->AddSwitchPort (port.first, port.second);
  }
