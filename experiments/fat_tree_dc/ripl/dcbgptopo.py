...
The modifications are required to reflect the current Mininet API. 
...
class BGPFatTreeTopo(StructuredTopo):
    ...
    def __init__(self, k = 4, speed = 1.0):
        ...
        super(BGPFatTreeTopo, self).__init__(node_specs, edge_specs)

        self.k = k
        self.id_gen = BGPFatTreeTopo.FatTreeNodeID
        self.numPods = k
        self.aggPerPod = k / 2
        ...
        for p in pods:
            for e in edge_sws:
                ...
                for a in agg_sws:
                    ...
                    edge_eth = [{'ipAddrs': ["10.1.%s.%s/24" % (edge_aggr_sub, 1)] }]
                    agg_eth = [{'ipAddrs': ["10.1.%s.%s/24" % (edge_aggr_sub, 2)]}]
                    ...
                    edge_neighbors.append ( {'address': edge_neighbor_ip,
                                           'as':agg_asn, 
                                           'local-address': agg_neighbor_ip} )
                    nodes[agg_id]['neighbors'].append( {'address': agg_neighbor_ip,
                                                      'as':edge_asn,
                                                      'local-address':edge_neighbor_ip} ) 
                    self.addLink(edge_id, agg_id, port1=src, port2=dst)
                    edge_aggr_sub += 1
                ...
                for h in hosts:
                    ...
                    self.addLink(host_id, edge_id, port1=dst, port2=src)
                    net_idx += 1
                    edge_ip += 1
            ...
            for a in agg_sws:
                ...
                for c in core_sws:
                    ...
                    agg_eth = [{'ipAddrs': ["20.1.%s.%s/24" % (agg_sub_net, 1)] }]
                    core_eth = [{'ipAddrs': ["20.1.%s.%s/24" % (agg_sub_net, 2)]}]
                    nodes[agg_id]['intfs'][agg_id + '-eth' + str(dst)] = agg_eth
                    ...
                    nodes[core_id]['neighbors'].append ( {'address': core_neigh_ip,
                                                        'as':nodes[agg_id]['asn'], 
                                                        'local-address':agg_neigh_ip} )
                    nodes[agg_id]['neighbors'].append( {'address':agg_neigh_ip,
                                                      'as':core_asn,
                                                      'local-address':core_neigh_ip} )
                    self.addLink(core_id, agg_id, port1=src, port2=dst)
                    agg_sub_net += 1
            agg_asn += 1
        ...
        for node in nodes:
            if 'asn' in nodes[node]:
                if nodes[node]['type'] == self.LAYER_EDGE or nodes[node]['type'] == self.LAYER_AGG: 
                    self.addHost(node,
                                    intfDict=nodes[node]['intfs'],
                                    asNum=nodes[node]['asn'],
                                    neighbors=nodes[node]['neighbors'],
                                    routes=nodes[node]['networks'],
                                    cls=BgpRouter,
                                    allowas_in = False,
                                    maximum_paths=str(self.k), speaker = "quagga",
                                    **nodes[node]['opts'])
                else:
                    self.addHost(node,
                                    intfDict=nodes[node]['intfs'],
                                    ...
                                    routes=nodes[node]['networks'],
                                    cls=BgpRouter,
                                    allowas_in = True, speaker = "quagga",
                                    **nodes[node]['opts'])
            else:
                self.addHost(node,
                            cls=SdnipHost,
                            ips=nodes[node]['ip'],
                            gateway=nodes[node]['gateway'], 
                            **nodes[node]['opts'])
...
# hosts = topo.layer_nodes(topo.LAYER_EDGES)
# print len(hosts)